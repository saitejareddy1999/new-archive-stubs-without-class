import os
import shutil#basically it performs with file operations,ability to copy files, move files,remove files this mainly occurrs in deletion confirmation sestions copy for separation remove for deletion
import sys
from datetime import datetime
import logging
import csv
#text is read me file and .url is archive stub file
def errHandle(filepathname):
    logging.basicConfig(filename='./errors/' + filepathname, level=logging.DEBUG, 
                    format='%(asctime)s %(levelname)s %(name)s %(message)s')
    
def getListOfFiles(dirName, optProc, confirmAct='No'):
    errHandle('ArchivedError.log')
    ...
    for entry in sorted(listOfFile):
        ...
        if ('ARCHIVE STUB' in entry.upper() or 'ARCHIVESTUB' in entry.upper()) and entry != 'Archived_File_Stubs' and entry.endswith('.url'):
            StubFile_Count +=1
        #if we take separation we will take this
        # If entry is a directory then get the list of files in this directory 
        if os.path.isdir(fullPath) and entry != 'Archived_File_Stubs':
            allFiles = allFiles + getListOfFiles(fullPath, optProc, confirmAct)
            if entry == 'Archived_File_Stubs':
                allFiles.append(fullPath)
        else: #separation will come in this area           
            ##remove readme file
            ...
            ##1)Remove archived stub file and read me file (.url & .txt)
...
def getListOfFilesOneLevel(dirName, optProc, confirmAct='No'):
    errHandle('ArchivedError.log')
    # create a list of file and sub directories 
    # names in the given directory 
    listOfFile = os.listdir(dirName)
    ...
    return allFiles
def getCount(FileListData, folderPath, optProc):
    StubFile_Count = 0
    ReadMe_Count = 0
    StubFolder_Count = 0
    dict_counter = {}
