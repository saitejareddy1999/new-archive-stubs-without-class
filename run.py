...
from stubs import getListOfFiles, getCount, getListOfFilesOneLevel
...
def ajaxmovestubs():
    if request.method == 'POST':
        ...
        start = time.time()
        if noSubFolders == 'YES':
            FileData =  getListOfFilesOneLevel(folderPath, optProc)#getListOfFilesOneLevel this is for it will read only one file and delete or separate unwanted data
        else:
            FileData =  getListOfFiles(folderPath, optProc)
        end = time.time()
        processTime = end-start 
        s=round(processTime,1)
        print(s)
        # FileData =  getStubsStale(folderPath)
        CountData = getCount(FileData, folderPath, optProc)
...
def stubsconfirm():
    if request.method == 'POST':
        ...
        start = time.time()
        if noSubFolders == 'YES':
            CountData =  getListOfFilesOneLevel(folderPath, optProc, 'Yes')#yes means for deletion confirmation will trigger,getListOfFilesOneLevel this is for it will read only one file and delete or separate unwanted data
        else:
            CountData =  getListOfFiles(folderPath, optProc, 'Yes')
        msg = f'{CountData}'
    return jsonify(msg)
